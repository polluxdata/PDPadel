...
export default function NewQuedadaPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    router.push(`/groups/${params.id}/quedadas/${quedada.id}`);
  }

  if (!group || !season) {
    return (
      <div>
        <AppHeader title="Nueva quedada" backHref={`/groups/${params.id}`} />
  ...
  return (
    <div>
      <AppHeader title="Nueva quedada" subtitle={season.name} backHref={`/groups/${params.id}`} />
            ...
            <div>
              ...
              {mode === 'points' ? (
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-1.5">
                    {SETS_OPTIONS.map((n) => (
                      <button key={n} type="button" onClick={() => setMaxSets(n)}
                        className={'rounded-lg px-3 py-1 text-sm font-bold ' + (maxSets === n ? 'bg-emerald-500 text-slate-950' : 'bg-slate-800 text-slate-300')}>
                        {n}
                      </button>
                    ))}
                  </div>

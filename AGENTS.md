...
- **Alta directa por super admin**: en el panel `/admin` hay un formulario "Dar de alta a un jugador" (solo super admin, `POST /api/admin/users`): captura nombre/usuario/email y crea la cuenta sin registro propio (`pin_hash` nulo), opcionalmente enviando un magic link de bienvenida. Pensado para jugadores mayores que no se registran solos.
